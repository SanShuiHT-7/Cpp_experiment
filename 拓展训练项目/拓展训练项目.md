...
```c++
void Book::setWriter(const float newWriter)
{  
    writer=newWriter;
}
```
...
```C++
string Temp1,float Temp2,string Temp3;
cout<<setw(20)<<"请输入图书的书名：";
cin>>Temp1;
b1.setName(Temp1);
cour<<setw(20)<<"请输入图书的单价：";
cin>>Temp2;
b1.setPrice(Temp2);
cour<<setw(20)<<"请输入图书的作者：";
cin>>Temp3;
b1.setPrice(Temp3);
cout<<setw(10)<<b1.get_name();				//swtw(10)设置输出域宽为10个字符
cout<<setw(10)<<b1.get_price();
cout<<setw(10)<<b1.get_writer();
```
